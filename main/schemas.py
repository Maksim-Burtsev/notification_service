...
class MessageSchema(Schema):
    ...
    dtime: datetime = Field(
        ..., description="Время создания (или отправки)", alias="datetime"
    )
...
class CountMessageSchema(Schema):
    ...
    success: int = Field(..., description="Успешно доставленные")
    wrong: int = Field(..., description="Недоставленные")
    waiting: int = Field(..., description="Ожидают отправки")
    trying: int = Field(..., description="Не удалось отправить с первого раза")
    total: int = Field(..., description="Всего сообщений в рассылке")
